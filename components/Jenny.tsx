/**
 * Jenny Vector Engine v2 — Living Mascot
 * ─────────────────────────────────────
 * Parametric SVG representation of Jenny the Penguin mascot.
 * Now with idle breathing animation & blink cycle for a living feel.
 */
...
export default function Jenny({ size = 80, mood = 'neutral', showLabel = false, animate = true }: JennyProps) {
    ...
    useEffect(() => {
        ...
        breatheScale.value = withRepeat(
            withTiming(1.04, { duration: 2500, easing: Easing.bezier(0.45, 0, 0.55, 1) }),
            -1,
            true
        );
        breatheTranslateY.value = withRepeat(
            withTiming(-3, { duration: 2500, easing: Easing.bezier(0.45, 0, 0.55, 1) }),
            -1,
            true
        );
    }, [animate]);
    ...
    // Adorable white-highlight-in-black-circle eye
    const CuteEye = ({ cx, cy }: { cx: number, cy: number }) => (
        <G>
            <Circle cx={cx} cy={cy} r="6" fill={Colors.ink} />
            <Circle cx={cx - 1.5} cy={cy - 1.5} r="2" fill={Colors.snow} />
        </G>
    );
    ...
    const renderFaceDetails = () => {
        switch (normalizedMood) {
            ...
            case 'neutral':
                return <><CuteEye cx={35} cy={45} /><CuteEye cx={65} cy={45} /><BeakNormal /></>;
            case 'restless':
            case 'curious':
                return <><CuteEye cx={34} cy={45} /><CuteEye cx={66} cy={45} /><BeakOpen /></>;
            case 'sad':
    ...
    return (
        <View style={[styles.container, { width: size + 8, height: showLabel ? size + 32 : size + 8 }]}>
            {animate ? (
            ...
            {showLabel && <Text style={styles.label}>Jenny</Text>}
        </View>
    );
}
